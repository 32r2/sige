...
//Usuarios por tienda
function Usuarios_X_Tienda(IDTienda, IDExtra, NombreExtra) {
...
function llenarBadges() {
    $.get("/Cardinal/BDAreas", function (DatosAreas) {
        for (var i = 0; i < DatosAreas.length; i++) {
            ...
            $.get("/Incidencias/ReportesXArea/?IDArea=" + IDArea, function (IncidenciasRpt) {
                if (IncidenciasRpt.length > 0) {
                    ...
                    Tabla += "</tbody>";
                    Tabla += "</table>";
                    document.getElementById("Tabla_Depto_" + IncidenciasRpt[0].IDArea).innerHTML = Tabla;
                }
            });
        }
    });
